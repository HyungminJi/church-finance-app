...
(async () => {
  ...
  try {
      ...
      console.log('waitForLoadState failed, continuing...');
  }
  ...
  if (bodyText.includes(menuText)) {
      ...
      try {
          ...
          await page.getByText(menuText, { exact: false }).first().click();
      ...
      // 메뉴를 찾지 못했으므로, 화면에 보이는 링크들 출력
      const links = await page.evaluate(() => Array.from(document.querySelectorAll('a')).map(a => a.innerText.trim()).filter(t => t.length > 0));
