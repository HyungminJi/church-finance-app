      ...
      <div class="h-[calc(100vh-16rem)] flex flex-col md:flex-row gap-4">
        ...
        <div class="p-3 bg-gray-50 dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center">
          <span class="text-sm font-black text-gray-700 dark:text-gray-300 flex items-center gap-2">
            <UIcon name="i-heroicons-tag" class="text-blue-500" />
            계정과목별 원장
          </span>
          <UButton 
            ...
            <li v-for="group in accountTree" :key="group.name" class="space-y-1">
              ...
              <ul v-if="group.expanded" class="pl-4 mt-1 border-l-2 border-gray-100 dark:border-gray-700 ml-4 space-y-0.5 animate-in slide-in-from-top-1 duration-200">
                <li 
                  ...
                  :class="{'bg-primary-500/10 font-bold text-blue-500 dark:text-blue-500 ring-1 ring-primary-500/20': selectedAccountCode === acc.code}"
                  @click="selectAccount(acc.code)"
                >
                   <UIcon name="i-heroicons-document-text" class="w-4 h-4 mr-2" :class="selectedAccountCode === acc.code ? 'text-blue-500' : 'text-gray-300'" />
        ...
        <div class="p-4 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/50 flex flex-col gap-4">
          ...
          <div class="flex justify-between items-end">
            <div class="text-xl font-black text-gray-900 dark:text-white flex items-center gap-2">
               <UIcon name="i-heroicons-book-open" class="text-blue-500 w-6 h-6" />
               {{ currentViewLabel }}
            ...
            <div v-if="meta" class="flex gap-6 text-right animate-in fade-in duration-500">
              <div class="group">
                ...
                <span class="text-base font-black text-blue-500 font-mono">{{ formatNumber(meta.period_income) }}</span>
              </div>
              <div class="group">
          ...
          <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700 border-collapse">
            ...
            <tbody class="divide-y divide-gray-100 dark:divide-gray-800 bg-white dark:bg-gray-800">
              <tr v-if="meta" class="bg-primary-50/30 dark:bg-primary-900/5 font-black border-b-2 border-primary-50 dark:border-primary-900/20">
                ...
                <td class="px-4 py-3 text-right text-sm text-blue-500 font-mono">{{ formatNumber(meta.previous_income) }}</td>
              ...
              <tr v-for="tx in transactions" :key="tx.id" class="hover:bg-primary-50/20 dark:hover:bg-primary-900/10 transition-colors border-l-4 border-transparent hover:border-primary-500">
                ...
                <td class="px-4 py-3 whitespace-nowrap text-sm text-right text-blue-500 font-mono font-black">
